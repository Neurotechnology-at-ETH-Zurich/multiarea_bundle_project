function fig = LFPViewer(data,samplerate,ripple_timestamps,ripple_centers,ripple_classes,structures,readonly)
...
% structures        (1,2)   string with the structures we want to analyze
...
appdata.ripple_classes = categorical(ripple_classes); % category of ripple events
assert(iscategorical(appdata.ripple_classes),'ripple_classes needs to be a categorical vector');
appdata.readonly = readonly;
appdata.structures = structures;
...
if appdata.readonly
    ...
    appdata.helpMessage = strcat("left/right arrow : prev/next frame, (shift) up/down arrow : prev/next (active) ripple, j : jump", ...
...
gl.RowHeight = {'fit','fit','1x','1x','1x','1x'};
gl.ColumnWidth = {'fit','1x','1x','fit','1x','1x'};
...
% dorsal hippocampus
fig_labelA = uilabel(gl,'Text',appdata.structures(1),'FontWeight','bold');
fig_axesA = uiaxes(gl);
...
fig_labelB = uilabel(gl,'Text',appdata.structures(2),'FontWeight','bold');
fig_axesB = uiaxes(gl);
fig_axesCWT4 = uiaxes(gl);
fig_axesCWT5 = uiaxes(gl);
fig_axesCWT6 = uiaxes(gl);

fig_axesCWT_slice4 = uiaxes(gl);
fig_axesCWT_slice5 = uiaxes(gl);
fig_axesCWT_slice6 = uiaxes(gl);

chan_listbox4 = uidropdown(gl,'Items',{'9', '10', '11', '12', '13', '14', '15', '16'});
chan_listbox5 = uidropdown(gl,'Items',{'9', '10', '11', '12', '13', '14', '15', '16'});
chan_listbox6 = uidropdown(gl,'Items',{'9', '10', '11', '12', '13', '14', '15', '16'});


%% Position Components
fig_label.Layout.Row = 1;
fig_label.Layout.Column = [1 6];

fig_listbox.Layout.Row = 3;
fig_listbox.Layout.Column = 1;

% dorsal hippocampus
fig_labelA.Layout.Row = 2;
fig_labelA.Layout.Column = 2;

fig_axesA.Layout.Row = 3;
fig_axesA.Layout.Column = [2 3];

fig_axesCWT.Layout.Row = 4;
fig_axesCWT.Layout.Column = 2;

fig_axesCWT_slice.Layout.Row = 4;
fig_axesCWT_slice.Layout.Column = 3;

fig_axesCWT2.Layout.Row = 5;
fig_axesCWT2.Layout.Column = 2;

fig_axesCWT_slice2.Layout.Row = 5;
fig_axesCWT_slice2.Layout.Column = 3;

fig_axesCWT3.Layout.Row = 6;
fig_axesCWT3.Layout.Column = 2;

fig_axesCWT_slice3.Layout.Row = 6;
fig_axesCWT_slice3.Layout.Column = 3;

chan_listbox.Layout.Row = 4;
chan_listbox.Layout.Column = 1;

chan_listbox2.Layout.Row = 5;
chan_listbox2.Layout.Column = 1;

chan_listbox3.Layout.Row = 6;
...
% intermediate hippocampus
fig_labelB.Layout.Row = 2;
fig_labelB.Layout.Column = 5;

fig_axesB.Layout.Row = 3;
fig_axesB.Layout.Column = [5 6];

fig_axesCWT4.Layout.Row = 4;
fig_axesCWT4.Layout.Column = 5;

fig_axesCWT_slice4.Layout.Row = 4;
fig_axesCWT_slice4.Layout.Column = 6;

fig_axesCWT5.Layout.Row = 5;
fig_axesCWT5.Layout.Column = 5;

fig_axesCWT_slice5.Layout.Row = 5;
fig_axesCWT_slice5.Layout.Column = 6;

fig_axesCWT6.Layout.Row = 6;
fig_axesCWT6.Layout.Column = 5;

fig_axesCWT_slice6.Layout.Row = 6;
fig_axesCWT_slice6.Layout.Column = 6;

chan_listbox4.Layout.Row = 4;
chan_listbox4.Layout.Column = 4;

chan_listbox5.Layout.Row = 5;
chan_listbox5.Layout.Column = 4;

chan_listbox6.Layout.Row = 6;
chan_listbox6.Layout.Column = 4;

channelsB = [9,9,9];
sel_channelsB = [9:16];

%% Initialize GUI
appdata.selectedAxes = fig_axesA; 

appdata.componentsA = {fig_axesA, fig_axesCWT, ...
    fig_axesCWT2, fig_axesCWT3, fig_axesCWT_slice, fig_axesCWT_slice2, ...
    fig_axesCWT_slice3, chan_listbox, chan_listbox2, chan_listbox3, channelsA, ...
    sel_channelsA};

appdata.componentsB = {fig_axesB, fig_axesCWT4, ...
    fig_axesCWT5, fig_axesCWT6, fig_axesCWT_slice4, fig_axesCWT_slice5, ...
    fig_axesCWT_slice6, chan_listbox4, chan_listbox5, chan_listbox6, channelsB, ...
    sel_channelsB};
...
drawLFP(fig,'fig_axesA'); % trigger inital plot
drawLFP(fig,'fig_axesB');

% App Behaviour
fig.WindowButtonDownFcn = {@(src,event) uiresume(src)}; % call uiresume on click
appdata.componentsA{1}.ButtonDownFcn = {@handleMouseClick,fig,'fig_axesA'};
appdata.componentsB{1}.ButtonDownFcn = {@handleMouseClick,fig,'fig_axesB'};
fig.KeyPressFcn = {@processKeyPress,fig_label,fig_listbox};

appdata.componentsA{8}.ValueChangedFcn = {@updateLFP,fig,'fig_axesA'};
appdata.componentsA{9}.ValueChangedFcn = {@updateLFP,fig,'fig_axesA'};
appdata.componentsA{10}.ValueChangedFcn = {@updateLFP,fig,'fig_axesA'};
appdata.componentsB{8}.ValueChangedFcn = {@updateLFP,fig,'fig_axesB'};
appdata.componentsB{9}.ValueChangedFcn = {@updateLFP,fig,'fig_axesB'};
appdata.componentsB{10}.ValueChangedFcn = {@updateLFP,fig,'fig_axesB'};

disp("Click on left or right axis to select them before adding or deleting events.");
disp("The name of each type of event needs to contain the name of the associated structure.");
end

%%
function drawLFP(src,axesName)
% drawLFP trigger update of the lfp plot
% src           the parent uifigure
% axesName      left or right axes

% unpack guidata, extract data in display interval and generate timestamps
appdata = guidata(src);
...
switch axesName
    case 'fig_axesA'
        uiaxes = appdata.componentsA{1};
        uiaxes2 = appdata.componentsA{2};
        uiaxes3 = appdata.componentsA{3};
        uiaxes4 = appdata.componentsA{4};
        uiaxes5 = appdata.componentsA{5};
        uiaxes6 = appdata.componentsA{6};
        uiaxes7 = appdata.componentsA{7};
        channels = appdata.componentsA{11};
        sel_channels = appdata.componentsA{12};
        structure = appdata.structures(1);
    
    case 'fig_axesB'
        uiaxes = appdata.componentsB{1};
        uiaxes2 = appdata.componentsB{2};
        uiaxes3 = appdata.componentsB{3};
        uiaxes4 = appdata.componentsB{4};
        uiaxes5 = appdata.componentsB{5};
        uiaxes6 = appdata.componentsB{6};
        uiaxes7 = appdata.componentsB{7};
        channels = appdata.componentsB{11};
        sel_channels = appdata.componentsB{12};
        structure = appdata.structures(2);
end

% select ripples by structure 
ripple_classes = appdata.ripple_classes(contains(string(appdata.ripple_classes),structure));
ripple_timestamps = appdata.ripple_timestamps(contains(string(appdata.ripple_classes),structure),:);
ripple_centers = appdata.ripple_centers(contains(string(appdata.ripple_classes),structure));

% verticaly space data by adding offset values
trace_offset = (1:appdata.n_channels/2) * appdata.trace_spacing;
...
plot_channels = [flip(sel_channels), flip(sel_channels+16), flip(sel_channels+32)]; % double the size of the traces
data_slice(:,plot_channels(1:8)) = 2 * data_slice(:,plot_channels(1:8));
plot(uiaxes,timestamps',double(data_slice(:,plot_channels))+offset,'Color',[0.5 0.5 0.5]);
xlabel(uiaxes,'time [s]');
xlim(uiaxes,[timestamps(1) timestamps(end)]);
ylabel(uiaxes,'');
ylim(uiaxes,[-500,4200]);

%fig_axesCWT
[cfs1,freq1] = cwt_mex(single(data_slice(:,channels(1))'),'amor',2000); % channels are no longer hard coded but based on selection
surf(uiaxes2,timestamps,freq1,(abs(cfs1).^2)./(1./double(freq1)),'FaceColor','interp','EdgeColor','none','FaceLighting','gouraud','PickableParts','none'); 
...
surf(uiaxes3,timestamps,freq2,(abs(cfs2).^2)./(1./double(freq2)),'FaceColor','interp','EdgeColor','none','FaceLighting','gouraud','PickableParts','none');
...
surf(uiaxes4,timestamps,freq3,(abs(cfs3).^2)./(1./double(freq3)),'FaceColor','interp','EdgeColor','none','FaceLighting','gouraud','PickableParts','none');
...
for i = 1:size(ripple_timestamps,1)
    ...
    % get interval and check visibility
    ...
    if (interval_frames(1)>=data_slice_start && interval_frames(1)<= data_slice_stop) ...
        ...
        text(uiaxes,xc(1),yc(1)+200,class,'Interpreter','none'); % display class label in top left corner
...
for index = visible_centers_index' % iterate over entries of row vector
    ...
    plot(uiaxes5,freq1,(abs(cfs1(:,hely(1))).^2)./(1./double(freq1)),'PickableParts','none');
    xlim(uiaxes5,[1 250])
    ...
    plot(uiaxes6,freq2,(abs(cfs2(:,hely(1))).^2)./(1./double(freq2)),'PickableParts','none');
    xlim(uiaxes6,[1 250])
    ...
    plot(uiaxes7,freq3,(abs(cfs3(:,hely(1))).^2)./(1./double(freq3)),'PickableParts','none');
    xlim(uiaxes7,[1 250])
...
%%
function processKeyPress(src,event,fig_label,fig_listbox)
...
interval_length = display_end - display_start;

switch event.Key
    case 'leftarrow' % shift frame back in time
        if display_start - interval_length >= 0
            % shift by display interval if possible
            display_start = display_start - interval_length;
            display_end = display_end - interval_length;
        else
            % otherwise just reset to start
            display_start = 0; %1;
            display_end = interval_length;
        end

    case 'rightarrow' % shift frame forward in time
        if display_end + interval_length <= appdata.framecount
            display_start = display_start + interval_length;
            display_end = display_end + interval_length;
        else
            display_start = appdata.framecount - interval_length;
    ...
    case {'uparrow','f'} % jump to the next (active) ripple event
        ...
        if next_ripple_delay ~= inf
            ...
            display_start = max([round(future - interval_length/2), 1]); % restrict interval to beginning if necessary
            display_end = min([display_start + interval_length, appdata.framecount]); % restrict interval to eof if we would extend past it
        end
    ...
    case 'downarrow' % jump to the previous (active) ripple event
        ...
        if next_ripple_delay ~= -inf
            ...
            display_start = max([round(past - interval_length/2),1]); % restrict interval to beginning if necessary
            display_end = min([display_start + interval_length, appdata.framecount]); % restrict interval to eof if we would extend past it
        end
    ...
    case 'a' % add ripple interval defined by start and end point
        ...
        if ~appdata.isSelectingEnd
            ...
            appdata.selectedStartTimestamp = appdata.selectedAxes.CurrentPoint(1,1);
            ...
            appdata.selectedEndTimestamp = appdata.selectedAxes.CurrentPoint(1,1);
    ...
    case 'd' % delete all ripple intervals containing target point
        ...
        src.Pointer = 'left';
        uiwait(src); % wait for click and get timestamp
        deletion_point = appdata.selectedAxes.CurrentPoint(1,1);
        % reset app state and pointer appearance
        appdata.isSelectingEnd = false;
        src.Pointer = 'arrow';
        fig_label.Text = appdata.helpMessage;
        ...
        for i = 1:size(appdata.ripple_timestamps,1)
            ...
            if deletion_point>=interval(1) && deletion_point<=interval(2) && appdata.ripple_classes(i)==fig_listbox.Value
    ...
    case 's' % save ripples to mat file
        ...
        ripple_classes = cellstr(char(appdata.ripple_classes));
        [savefile,savepath] = uiputfile('curated_ripples.mat');
        if savefile ~= 0
            savepath = fullfile(savepath,savefile);
            save(savepath,"ripple_timestamps","ripple_classes", '-v7');
    ...
    case 'l' % label all rippples containing target point with active class
        ...
        % get user input
        fig_label.Text = strcat("Click to label interval as"," ",fig_listbox.Value);
        src.Pointer = 'left';
        uiwait(src); % wait for click and get timestamp
        selection_point = appdata.selectedAxes.CurrentPoint(1,1);
        src.Pointer = 'arrow';
        fig_label.Text = appdata.helpMessage;
        % find overlaping ripples and gather indices for deletion
        to_label = [];
    ...
    case 'j' % jump to target time
        ...
        if answer >= 0 && answer <= (appdata.framecount / appdata.samplerate)
            ...
            display_start = max([round(targetFrame - interval_length/2), 1]); % restrict interval to beginning if necessary
            display_end = min([display_start + interval_length, appdata.framecount]); % restrict interval to eof if we would extend past it
        end
    ...
    case 'r' % rename active class
        ...
        if ~ismember(new_label,categories(appdata.ripple_classes))
            ...
            'LFPViewer rename : label already in use'
        end
...
% update the plot
drawLFP(src,'fig_axesA');
drawLFP(src,'fig_axesB');
end
...
%%
function updateLFP(src, event, fig, axesName)
% updateLFP update LFP view when the chosen channels change

appdata = guidata(fig);

switch axesName
    case 'fig_axesA'
        channels(1) = str2double(appdata.componentsA{8}.Value);
        channels(2) = str2double(appdata.componentsA{9}.Value);
        channels(3) = str2double(appdata.componentsA{10}.Value);
        appdata.componentsA{11} = channels;

    case 'fig_axesB'
        channels(1) = str2double(appdata.componentsB{8}.Value);
        channels(2) = str2double(appdata.componentsB{9}.Value);
        channels(3) = str2double(appdata.componentsB{10}.Value);
        appdata.componentsB{11} = channels;
end

guidata(fig,appdata);

drawLFP(fig,axesName);
end

%% 
function handleMouseClick(src, event, fig, axesName)

appdata = guidata(fig);
appdata.selectedAxes = src;
appdata.selectedAxesName = axesName;
