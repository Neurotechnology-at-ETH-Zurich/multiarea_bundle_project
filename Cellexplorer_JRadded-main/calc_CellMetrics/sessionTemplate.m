...
if ~isfield(session,'animal')
    question_ans= inputdlg('What animal are your using');
    session.animal.name = question_ans{1}; % Animal name is asked
...
if ~isfield(session,'spikeSorting')
    ...
    elseif ~isempty(dir(fullfile(basepath,relativePath,'TT*.mat')))
        ...
        session.spikeSorting{1}.manuallyCurated = 1;
        session.spikeSorting{1}.notes = '';
    elseif ~isempty(dir(fullfile(basepath,'JRCLUST-4.1.0')))
        disp('Spike sorting data detected: JRClust')
        session.spikeSorting{1}.relativePath = relativePath;
        session.spikeSorting{1}.format = 'jrclust';
        session.spikeSorting{1}.method = 'jrclust';
        session.spikeSorting{1}.channels = [];
        session.spikeSorting{1}.manuallyCurated = 0;
        session.spikeSorting{1}.notes = '';
    end
end
...
if ~isfield(session,'extracellular') && ~isfield(session.extracellular,'chanCoords') 
    if ~isempty(dir(fullfile(basepath,relativePath,'JRCLUST-4.1.0')))
        session.extracellular.chanCoords.layout = 'read_from_prm';
