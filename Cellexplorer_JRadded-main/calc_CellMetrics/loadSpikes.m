...
addParameter(p,'format','jrclust',@ischar); % clustering format: phy, klustakwik/neurosuite, KlustaViewa, NWB, Wave_clus, MClust, UltraMegaSort2000, ALF, AllenSDK
...
if parameters.forceReload
    ...
    switch lower(format)
        ...
        case 'jrclust'
               ...
               prmdata = readprm([basename,'.prm']);
            
               spikes = struct;
            
               %% one liners (done)
               spikes.basename=basename;
               spikes.numcells=cell_num;
               spikes.UID= linspace(1,cell_num,cell_num);
               spikes.cluID = linspace(1,cell_num,cell_num);
               spikes.sr=20000;
               wf_length = spike_file.rawShape(1);
               
               %% finding ids of spikes in clusters (done)
               spike_clu = spike_file.spikeClusters;
               spike_clu(find(spike_clu==0)) = spike_clu(find(spike_clu==0))-1;
               spike_clu(find(spike_clu==-1))= NaN;
               spike_file.spikeClusters = spike_clu;
               spikes.ids = cell(1,cell_num);
               for i = 1:cell_num
                    spikes.ids{i} = double(find(spike_file.spikeClusters == i)); 
                end
                
               %% assigning time to spikes notice: in cell explorer time is in seconds (done)
                spikes.times= cell(1,cell_num);
                spikes.ts= cell(1,cell_num);
                spikes.total = zeros(1,cell_num);
                for i = 1:cell_num
                    spikes.ts{i} = double(spike_file.spikeTimes(spike_file.spikesByCluster{i}));
                    spikes.times{i} = spikes.ts{i}./spikes.sr;
                    spikes.total(i) = length(spikes.ids{i});
                end
            
               
               %% assign shank to sites and calculate total (done)
              ...
              %---------------------------------------
               ...
               spikes.shankID=zeros(1,cell_num);
            
               for i = 1:cell_num
                    spikes.shankID(i) = shankMap(spike_file.clusterSites(i)); 
               end
               
%                %% spindices (done)
%                spikes.spindices = zeros(spikes_num,2);
%                spikes.spindices(:,1) = spike_file.spikeTimes./spikes.sr;
%                spikes.spindices(:,2) = spike_file.spikeClusters;
               
               
               %% time waveforms (done)
               %----------for now, assume time starts at 0-------------
               spikes.timeWaveform = cell(1,cell_num);
               evtWindow = prmdata{4};
               for i = 1:cell_num
                   spikes.timeWaveform{i} = linspace(evtWindow(1),evtWindow(2),wf_length);
               end
               spikes.timeWaveform_all = spikes.timeWaveform;
            
               %% maxWaveform (done)
               spikes.maxWaveformCh1 = siteMap(spike_file.clusterSites);
               spikes.maxWaveformCh = spikes.maxWaveformCh1 -1;
               
               %load channel map
               spikes.maxWaveform_all = cell(1,cell_num);
               for i = 1:cell_num
                   spikes.maxWaveform_all{i} = siteMap;
               end
               %% rawWaveform
               spikes.rawWaveform= cell(1,cell_num);
               spikes.rawWaveform_std= cell(1,cell_num);
               spikes.rawWaveform_all= cell(1,cell_num);
               waveform_shape = size(squeeze(spike_file.meanWfGlobalRaw(:,:,1)));
               for l = 1:cell_num
                   spikes.rawWaveform{l} = double(squeeze(spike_file.meanWfLocalRaw(:,1,l))');
                   spikes.rawWaveform_all{l} = double(squeeze(spike_file.meanWfGlobalRaw(:,:,l))');
                for i = 1:waveform_shape(1)
                   spikes.rawWaveform_std{l}(i) = double(std(spike_file.meanWfLocalRaw(i,:,l)));
                end
               end
                
               %% this part can be problematic, because JRclust doesn't seem to provide raw wave form. 
               spikes.filtWaveform= cell(1,cell_num);
               spikes.filtWaveform_std= cell(1,cell_num);
               spikes.filtWaveform_all= cell(1,cell_num);
               waveform_shape = size(squeeze(spike_file.meanWfGlobal(:,:,1)));
               for l = 1:cell_num
                   spikes.filtWaveform{l} = double(squeeze(spike_file.meanWfLocal(:,1,l))');
                   spikes.filtWaveform_all{l} = double(squeeze(spike_file.meanWfGlobal(:,:,l))');
                for i = 1:waveform_shape(1)
                   spikes.filtWaveform_std{l}(i) = double(std(spike_file.meanWfLocal(i,:,l)));
                end
               end
            
               %% peak voltage (done)
               spikes.peakVoltage = double(spike_file.unitPeaks');
               
               %% channels_all for some reason
               spikes.channels_all = cell(1,cell_num);
               for i = 1:cell_num
