...
% Last Modified by GUIDE v2.5 21-May-2022 00:05:55
...
p = plot3(handles.trace.trace_mat(:,1),handles.trace.trace_mat(:,2),repmat(min(min(handles.hmaps.hmaps{handles.cell_id}))-100,length(handles.trace.trace_mat(:,1)),1),'r');
xlim([0,700])
ylim([0,600])
hold on
s = surf(handles.x,handles.y,handles.hmaps.hmaps{handles.cell_id},'FaceColor','interp','EdgeColor','none','FaceLighting','gouraud');
view(2)
alpha(0.8)
hold off
...
% handles    structure with handles and user data (see GUIDATA)
ids = handles.cell_id;
ids = rem(ids+1,handles.numcells);
handles.cell_id = ids;
set(handles.id,'string',handles.cell_id);
p = plot3(handles.trace.trace_mat(:,1),handles.trace.trace_mat(:,2),repmat(min(min(handles.hmaps.hmaps{handles.cell_id}))-100,length(handles.trace.trace_mat(:,1)),1),'r');
xlim([0,700])
